...
summary: Sometimes you just want to view certain parts of YouTube video. Skyppy makes it easy for you to see who's doing all the talking and to skip the parts you don't want.
---

## [Skyppy](https://skyppy.tv)
...
I knew that I had to base our next Bad Idea™ on it. INA’s Speech Segmenter segments audio into music, silence, noise and speaker gender. Turns out that last bit needs to be approached carefully.

We unashamedly stole the name from a [BBC Newslabs project](https://bbcnewslabs.co.uk/news/2020/skippy-hack-to-home/) but changed the spelling to avoid any conflict.

Like all side projects I work on, they tend to last years (if not decades). I think we’re into our 3rd year with [Skyppy](https://skyppy.tv). We've been in no hurry, so it’s been a slow, slow burner that we fitted in between paid work. 

So what’s the idea behind [Skyppy](https://skyppy.tv)? The idea is to segment or split parts of an audio file (or YouTube video in our case), based on specific criteria – silence, voice type, music and noise. Maybe you don’t want to listen to that intro music or the applause and just consume the parts that matter. Or maybe you just want a handy visual guide of who is doing all the speaking. Or like me, you prefer to [remove voices while leaving the pregnant pauses in for comic effect](https://skyppy.tv/?#v=udVvb5giPcg&s=hqn&h=Chigusa&l=Robert).

I started working on this with my friend [Marco](https://github.com/scarsellifi/), who handled all the back-end stuff, while I concentrated on the front-end and pretty early on in the development process, [we had a demo…](
...
Also sometimes the algorithm gets things wrong, so you can re-categorise segments by "long pressing" the segment. All of these changes are stored on the URL so you can share a customised version with others.

## Comic Effect

For demo purposes I used a Robert Smith (from the band The Cure) being interviewed after a concert in Japan and just removed his voice, this had a comical effect as his mannerisms were left in and looked like non verbal responses to the questions. I’d like to repeat (as I did on Twitter), that was not meant as a slight, I think Robert is a wonderful person. [You can check this out in the latest version here!](https://skyppy.tv/?#v=udVvb5giPcg&s=hqn&h=Chigusa&l=Robert)
...
Although one limitation I did have was with the backend. Most AI/ML based systems are written in Python and I have limited experience of Python, so I asked my good friend and neighbour (we live in the same town at least) to give me a hand. Marco also knows a fair bit about various cloud platforms and how to package things up to make them easily deployable and scalable. And besides, when you commit to something with somebody else, you can motivate each other to keep going (turns out this is super important for us).

## Slow Code

I live in Italy which gave birth to the Slow Food Movement, and [Skyppy](https://skyppy.tv) is a classic example of something you could call the Slow CodeMovement. Not code that runs slow, but a slower process of writing code that lets you actually enjoy it, without time pressure. Working on [Skyppy](https://skyppy.tv) for me, was a way to relax and an excuse to socialise. Nevertheless after three years, I’m happy to say we’re ready to release the first version.


## Technical Considerations

Machine learning algorithms are usually processor intensive. In other words – the cost of running them can become significant, especially in the cloud. So we soon realised that if we were going to put a demo online, we’d have to restrict the amount of content people could submit, somehow.
...
The other restriction we put in place is that we only accept videos of length 15 minutes or under.
...
We don’t store any identifying data such as IP addresses, but we are curious about how [Skyppy](https://skyppy.tv) is used and so we keep stats on…
...
Although we have plans to add client-side tests, these have not yet been created. We do have server-side tests though, using [PyTest](https://pytest.org). It would be optimistic to say that we’ve achieved 89% coverage with these tests, but hey, we’re optimistic people.

## Developer Experience

As [Skyppy](https://skyppy.tv) has a number of moving parts split between client and server, we found it easiest to encapsulate this all in a Docker container. This also helps make the deployment process more trivial. The slight downside is that it’s not always easy to get that seamless hot-reloadable developer experience. And on a Mac at least I need to restart the docker instance after every change. It also takes a while to build the very first time you run it.
...
In an attempt to provide a better experience we experimented with a version based on the Python package and dependency management tool known as [Poetry](https://python-poetry.org/) Although this seems to work fine on Linux and Windows we had issues on MacOS. Your mileage may vary. More details can be found in the documentation.
